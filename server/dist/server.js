...
io.on("connection", (socket) => {
    ...
    socket.on("room:create", ({ roomName, userName, userId }) => {
    ...
    socket.on("message:send", (data) => {
        ...
        if (!roomsData[roomId])
            return;
        const newMessage = { id: Date.now(), user, content, timestamp: new Date().toISOString() };
        roomsData[roomId].messages.push(newMessage);
        io.to(roomId).emit("message:new", { message: newMessage });
    });
    const isHost = (room, userId) => room && room.host.userId === userId;
    socket.on("game:start", ({ roomId, userId }) => {
        const room = roomsData[roomId];
    ...
    socket.on("question:submit", ({ roomId, question, userId }) => {
        const room = roomsData[roomId];
        if (isHost(room, userId)) {
    ...
    socket.on("answer:submit", ({ roomId, answer, userId }) => {
        const room = roomsData[roomId];
    ...
    socket.on("vote:submit", ({ roomId, answerId, guessedUserId, voterUserId }) => {
        const room = roomsData[roomId];
        if (!room || room.gameState !== "perAnswerVoting")
            return;
        const currentAnswer = room.answers[room.currentAnswerIndex];
        if (!currentAnswer || currentAnswer.id !== answerId)
            return; // 現在投票中の回答でなければ無視
        // 投票を記録
    ...
    socket.on("game:nextAnswer", ({ roomId, userId }) => {
        const room = roomsData[roomId];
        if (!isHost(room, userId) || room.gameState !== "perAnswerRevealing")
            return; // ホストのみ、かつ結果発表フェーズのみ
    ...
    socket.on("game:nextRound", ({ roomId, userId }) => {
        const room = roomsData[roomId];
        if (isHost(room, userId)) {
    ...
    socket.on("room:disband", ({ roomId, userId }) => {
        const room = roomsData[roomId];
        if (isHost(room, userId)) {
